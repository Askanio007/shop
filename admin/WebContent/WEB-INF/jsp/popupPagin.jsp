		...
		<tr class = "pagin">
			<c:forEach begin="1" end="${pagination.countPage}" var="i">
   				<c:choose>
            		...
            		<c:otherwise>
   						<td class = "pagin"><a href="#" onclick="viewBuyerPagin(${param.page},${i})">${i}</a></td>
   					</c:otherwise>
   				</c:choose>
   			</c:forEach>
   		</tr>
     			...
     			<c:when test="${pagination.currentPage eq 1}">
 					<tr class = "pagin">
        				<c:forEach begin="${pagination.currentPage}" end="${pagination.currentPage+2}" var="i">
   						...
   						<td class = "pagin">...</td>
   						<td class = "pagin"><a href="#" onclick="viewBuyerPagin(${param.page},${i})">${pagination.countPage}</a></td>
   					</tr>
   				</c:when>
   				...
   				<c:when test="${pagination.currentPage eq pagination.countPage}">
   					<tr class = "pagin">
   					<td class = "pagin"><a href="#" onclick="viewBuyerPagin(${param.page},1)">1</a></td>
   					<td class = "pagin">...</td>
				...
				<c:otherwise>
   					<tr class = "pagin">
   						...
   						<c:if test="${pagination.currentPage le pagination.countPage-2}">
   							<td class = "pagin"><a href="#" onclick="viewBuyerPagin(${param.page},${pagination.countPage})">${pagination.countPage}</a></td>
   						</c:if>
					</tr>
				</c:otherwise>
			</c:choose>
